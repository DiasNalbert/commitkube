...
func ListWorkspaces(c *fiber.Ctx) error {
	...
	if callerIsAdmin(c) {
		...
		var wsIDs []uint
		if len(groupIDs) > 0 {
			db.DB.Model(&models.UserGroupWorkspace{}).Where("group_id IN ?", groupIDs).Pluck("workspace_id", &wsIDs)
		}
		if len(wsIDs) > 0 {
			db.DB.Where("id IN ?", wsIDs).Find(&workspaces)
		}
	}
...
func CreateWorkspace(c *fiber.Ctx) error {
	if !callerIsAdmin(c) {
		return c.Status(fiber.StatusForbidden).JSON(fiber.Map{"error": "Only admins can create workspaces"})
	}
	userID := uint(c.Locals("user_id").(float64))
	...
	var existing models.BitbucketWorkspace
	if db.DB.Where("workspace_id = ?", req.WorkspaceID).First(&existing).Error == nil {
		return c.Status(fiber.StatusConflict).JSON(fiber.Map{"error": "workspace " + req.WorkspaceID + " já existe (ID " + fmt.Sprintf("%d", existing.ID) + ")"})
	}
...
func UpdateWorkspace(c *fiber.Ctx) error {
	if !callerIsAdmin(c) {
		return c.Status(fiber.StatusForbidden).JSON(fiber.Map{"error": "Only admins can edit workspaces"})
	}
	id := c.Params("id")

	var ws models.BitbucketWorkspace
	if err := db.DB.Where("id = ?", id).First(&ws).Error; err != nil {
...
func DeleteWorkspace(c *fiber.Ctx) error {
	if !callerIsAdmin(c) {
		return c.Status(fiber.StatusForbidden).JSON(fiber.Map{"error": "Only admins can delete workspaces"})
	}
	id := c.Params("id")
	...
	wsID := ws.ID
	db.DB.Delete(&ws)
	db.DB.Where("workspace_id = ?", wsID).Delete(&models.YamlTemplate{})
	db.DB.Where("workspace_id = ?", wsID).Delete(&models.GlobalVariable{})
	db.DB.Where("workspace_id = ?", wsID).Delete(&models.BitbucketProject{})
