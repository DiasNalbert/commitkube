...
export default function UsersPage() {
  ...
  const submitResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      const res = await apiFetch(`/users/${resetPwdUser.id}/password`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      {resetPwdUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="glass-card p-6 w-full max-w-sm space-y-4">
            ...
            <p className="text-sm text-zinc-400">New password for <span className="text-zinc-200 font-mono">{resetPwdUser.email}</span></p>
            {resetPwdError && (
              <div className="p-3 rounded bg-red-500/10 border border-red-500/30 text-red-400 text-sm">{resetPwdError}</div>
            )}
            <form onSubmit={submitResetPassword} className="space-y-3">
              ...
              <div className="flex gap-3 justify-end">
                <button type="button" onClick={() => { setResetPwdUser(null); setResetPwdValue(""); setResetPwdError(""); }} className="text-zinc-400 hover:text-zinc-200 text-sm">
                  Cancel
                </button>
                <button type="submit" disabled={resetPwdLoading} className="btn-primary py-1.5 px-4 text-sm">
                  {resetPwdLoading ? "Saving..." : "Reset"}
                </button>
              </div>
            </form>
          </div>
